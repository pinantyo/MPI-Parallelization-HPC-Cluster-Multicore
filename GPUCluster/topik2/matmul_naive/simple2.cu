...
void MatrixMultiplication(int THREADSX, int THREADSY, double *a, double *mul, int m, int n) {
...
int main(int argc,char* argv[]) {
	...
	if (argc == 5){
		m = atoi(argv[1]);
		n = atoi(argv[2]);
		THREADSX = atoi(argv[3]);
		THREADSY = atoi(argv[4]);
	}
	else{
	...
	printf("\tStart1\n" );
	MatrixMultiplication(THREADSX, THREADSY, A, C, m, n);
