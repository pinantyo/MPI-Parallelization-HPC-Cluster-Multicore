...
double communication_time, coordinate_time, process_time, total_communication_time;
...
int main(int argc, char *argv[]) {
  ...
  if (myid == 0) {
    ...
    double coordinate_time = MPI_Wtime();
    Coordinator(numWorkers, stripSize, gridSize);
    printf("Coordinator time %f \n", MPI_Wtime()-coordinate_time);
    printf("Process time %f \n", MPI_Wtime()-process_time);
...
static void Coordinator(int numWorkers, int stripSize, int gridSize) {
  ...
  for (workerid = 1; workerid <= numWorkers; workerid++) {
    ...
    //printf("got results from worker: %d and Recv time: %f \n", workerid, communication_time);
  }
  ...
  communication_time = MPI_Wtime()-communication_time;
  total_communication_time += communication_time;
  //printf("global maxdiff is %f and Reduce time: %f\n", maxdiff, communication_time);
...
static void Worker(int myid, int numWorkers, int stripSize,
  ...
  for (iters = 1; iters <= numIters; iters++) {
    ...
    communication_time = MPI_Wtime()-communication_time;
    total_communication_time += communication_time;
    //printf("Workers MPI_Send Communication Time: %f\n", communication_time);
    /* update my points */
  ...
  communication_time = MPI_Wtime()-communication_time;
  total_communication_time += communication_time;
  //printf("Workers MPI_Send2 Communication Time: %f\n", communication_time);
  ...
  communication_time = MPI_Wtime()-communication_time;
  total_communication_time += communication_time;
  //printf("maxdiff of worker %d is %f and time %f\n", myid, mydiff, communication_time);
}
