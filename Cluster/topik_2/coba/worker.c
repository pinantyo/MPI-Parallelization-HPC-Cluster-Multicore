...
int main( int argc, char *argv[] )
{
    ...
    double b[SIZE][SIZE], c[SIZE][SIZE], d[SIZE][SIZE];
    MPI_Bcast(b, SIZE*SIZE, MPI_DOUBLE, 0, parentcomm );

    if (myrank < SIZE) {
        MPI_Recv(&c, rows*SIZE, MPI_DOUBLE, 0, MPI_ANY_TAG, parentcomm, &status);
        while ( status.MPI_TAG > 0 ) {
            ...
            //     }
            // }
            MPI_Send(&d, rows*SIZE, MPI_DOUBLE, 0, row+1, parentcomm);
            MPI_Recv(&c, rows*SIZE, MPI_DOUBLE, 0, MPI_ANY_TAG, parentcomm, &status);
