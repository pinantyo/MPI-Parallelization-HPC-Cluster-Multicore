...
int main( int argc, char *argv[] )
{
   ...
   //double a[SIZE][SIZE],b[SIZE][SIZE],c[SIZE][SIZE]
    ...
    MPI_Bcast( &b[0][0], SIZE*SIZE, MPI_DOUBLE, MPI_ROOT, workercomm );
    ...
    for ( i = 0; i < MIN( numworkers, SIZE ); i++ ) {
	    MPI_Send( &a[numsent][0], SIZE, MPI_DOUBLE, i, i+1, workercomm );
        numsent = numsent++;
    }
    comm_time += MPI_Wtime();
    ...
    for ( i = 0; i < SIZE; i++ ) {
        comm_time -= MPI_Wtime();
        MPI_Recv( &c[i][0], SIZE , MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, workercomm, &status );
        comm_time += MPI_Wtime();
        ...
        if ( numsent < SIZE ) {
            comm_time -= MPI_Wtime();
            MPI_Send( &a[numsent][0], SIZE, MPI_DOUBLE, sender, numsent+1, workercomm );
            comm_time += MPI_Wtime();
            numsent++;
        } else { /* no more work */
            comm_time -= MPI_Wtime();
